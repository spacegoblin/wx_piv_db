...
DB = db.This_Db(const.db_lst_dsn, 'ahetland', pwd.pwd('alhet'))
   ...
   # try:
    for row in reader:
        ...
        if i==0:
            ...
            dict_['ust'] =  row[6].replace(',', '.')
            dict_['belegfeld1'] = row[7].strip()
            dict_['soll'] = row[8].replace('.', '').replace(',', '.')
            dict_['haben'] = row[9].replace('.', '').replace(',', '.')
            dict_['wkz'] = row[10]
            #dict_['xx'] = row[11]
            dict_['kurs'] = row[12].replace(',', '.')
            ...
            dict_['imp_str'] = unicode(const.random_str, errors='ignore')
            
            dict_['zui'] = unicode(const.random_str, errors='ignore')
...
if __name__ == '__main__':
   ...
   #T:\Reporting\2014\08-Aug\LSE
    PATH='L:\\Reporting\\2014\\11-Nov\\LSE\\imp.csv'
